...
- `lsm_db`: Chứa 50 thư mục, mỗi thư mục chứa 1 database có số record bằng với tên thư mục. 
    + `100000`: Btree database có 100 000 record.
    + ...................
    + `10000000`: Btree database có 10 000 000 record.
...
- Makefile có tổng cộng 16 nhãn, mỗi nhãn có thể được gọi để thực hiện một công việc, tất cả các nhãn đều cần được truyền tham só:
    ...
    - `build_cpp`: build file mã nguồn có tên gióng với `OP`.
...
- Cấu trúc của cây thư mục hiện tại dùng để chạy với các giá trị mặc đinh, nếu muốn chạy vơi các kích thước và giới hạn khác cần xóa 3 thư mục `btree_db`, `lsm_db`, `result` trước khi chạy.
...
- Sau khi kiểm tra và sửa lại các test case lỗi, dùng file `make_csv` để tổng hợp toàn bộ kết quả và ghi ra file csv:
